...
def run_arm(cfg: EcoConfig, mode: str, ticks: int, seed: int = 0,
            weights: EcoWeights | None = None) -> dict:
    """One matched-compute rollout; returns observables (read-only, P2).

    ``weights=None`` uses a fresh random θ from ``seed`` (the untrained control);
    pass an evolved ``EcoWeights`` (E2) to ablate the LEARNED chemistry. The init
    population is seeded by ``cfg.seed`` — vary it via ``dataclasses.replace`` to
    average an arm over held-out initial populations."""
    policy = make_attention_policy(cfg, seed=seed, mode=mode, weights=weights)
    eng = EcoEngine(cfg, policy=policy)
    max_resid = 0.0
    entropies, degrees = [], []
    survived = ticks
    pop_ticks = float(eng.state.n)
    ...
    return {
        ...
        "survived": survived,
        "final_n": eng.state.n,
        "pop_ticks": pop_ticks,                  # Σ_t n_t — the E2 fitness metric
...
def run_all(cfg: EcoConfig, ticks: int, seed: int = 0,
            weights: EcoWeights | None = None) -> list[dict]:
    return [run_arm(cfg, mode, ticks, seed, weights=weights) for mode in MODES]
