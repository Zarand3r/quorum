...
def main(argv=None):
    ...
    if RESULTS.exists():
        for l in RESULTS.read_text().splitlines()[1:]:
            f = l.split("\t")
            done.add((f[0], int(f[1])))
    arms = [x.strip() for x in a.arm.split(",") if x.strip()]
    todo = [(ar, sd) for ar in arms for sd in range(a.seed0, a.seed0 + a.seeds)
    ...
    with ProcessPoolExecutor(max_workers=a.workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            try:
                ...
                print(f"  {ar} sd={sd}: FAILED {exc!r}", flush=True)
                continue
            _append(row)
            print(f"  {ar} sd={sd}: vesicle_ckpts={row['vesicle_ckpts']} "
